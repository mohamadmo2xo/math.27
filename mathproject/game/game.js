...
(function () {
  ...
  const state = {
    ...
    shake: 0,
    lastTimerUpdate: 0,
    bestTime: parseFloat(localStorage.getItem(BEST_KEY) || '0') || 0
  };
  ...
  function sndStart() {
    ...
    [523.25, 659.25, 783.99].forEach(function (f, i) {
      setTimeout(function () { playTone(f, 0.18, 'sine', 0.14); }, i * 90);
    });
  }

  // صوت ظهور رمز
  function sndSpawn() {
    playTone(1100, 0.06, 'sine', 0.035, 800);
  }

  // صوت الاصطدام
  function sndCollision() {
    playTone(180, 0.22, 'sawtooth', 0.17, 80);
    playTone(90, 0.28, 'sine', 0.11);
  }

  // صوت الخسارة
  function sndGameOver() {
    [440, 392, 330, 262, 196].forEach(function (f, i) {
      setTimeout(function () { playTone(f, 0.28, 'sine', 0.15); }, i * 130);
    });
  }
  ...
  function sndLevelUp() {
    [660, 880, 1100].forEach(function (f, i) {
      setTimeout(function () { playTone(f, 0.12, 'triangle', 0.1); }, i * 70);
    });
  }
  ...
  function spawnEnemy() {
    ...
    state.enemies.push({
      ...
      symbol: sym.ch,
      color: sym.color,
      speed: state.baseSpeed * (0.85 + Math.random() * 0.4),
      life: lifetime,
      maxLife: lifetime,
      phase: Math.random() * Math.PI * 2,
      wander: Math.random() < 0.5 ? 1 : 0,
  ...
  function update(dt) {
    ...
    if (newLevel > state.level) {
      ...
      state.spawnInterval = Math.max(1.0, 2.5 - newLevel * 0.15);
    ...
    // تحديث المؤقت (بدون كل إطار)
    if (state.elapsed - state.lastTimerUpdate > 0.08) {
    ...
    // أثر اللاعب
    state.trail.push({ x: p.x, y: p.y, life: 0.35 });
    ...
    for (let i = state.enemies.length - 1; i >= 0; i--) {
      ...
      // نمط تمايل
      if (e.wander) {
        const perpX = -ey / edist;
        const perpY = ex / edist;
  ...
  function render() {
    ...
    for (let i = 0; i < state.trail.length; i++) {
      ...
      ctx.beginPath();
      ctx.arc(t.x, t.y, PLAYER_R * 0.75, 0, Math.PI * 2);
    ...
    for (let i = 0; i < state.enemies.length; i++) {
      const e = state.enemies[i];
      let alpha = 1;
      if (e.life < 1) alpha = 0.3 + 0.7 * Math.abs(Math.sin(e.life * 15));
      ...
      const fontSize = e.symbol.length > 1 ? e.r * 0.85 : e.r * 1.1;
  ...
  function updateHearts(animate) {
    const hearts = Math.max(0, state.hearts);
